...
namespace EmeraldEngine {
	...
	void InternalWindow::renderQuad() const {
		glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
	}

	void InternalWindow::renderQuads(unsigned int amount) const {
		glDrawArraysInstanced(GL_TRIANGLE_FAN, 0, 4, amount);
	}


	void InternalWindow::setBackgroundColor(float red, float green, float blue) {
		glClearColor(red, green, blue, 1.0f);
	}

	void InternalWindow::useFilter(std::weak_ptr<Shader> filter) {
		postProcessorUsed = true;
		postProcessingShader = filter;
	}

	void InternalWindow::clearFilter() {
	...
	void InternalWindow::resetTime() {
		glfwSetTime(0.0);
	}

	double InternalWindow::getTime() const {
		return glfwGetTime();
	}


	void InternalWindow::preUpdate() {
	...
	void InternalWindow::postUpdate() {
		if (postProcessorUsed) {
	...
	bool InternalWindow::continueRunning() {
		return !glfwWindowShouldClose(window);
	}


	bool InternalWindow::isKeyPressed(Key keyCode) const {
		return glfwGetKey(window, keyCode) == GLFW_PRESS;
	}

	bool InternalWindow::isMouseButtonPressed(MouseButton mouseButtonCode) const {
		return glfwGetMouseButton(window, mouseButtonCode) == GLFW_PRESS;
	}

	CursorPosition InternalWindow::getCursorPosition() const {
	...
	void InternalWindow::resize(unsigned int width, unsigned int height) {
		glfwSetWindowSize(window, width, height);
	}

	void InternalWindow::setTitle(std::string title) {
